...
MOD = 10007 # 나중에 10007로 나눈 나머지 출력할 것 

def fill_tile(n):
    if n == 1:  # 1일 때는 바로 값 리턴 (인덱스 에러 방지)
        return 1 # 너비 1일 때는 경우의 수 1개임 

    dp = [0] * (n + 1) # 방법의 수를 저장할 배열 

    dp[1] = 1 # 너비 1일 때는 경우의 수 1개임 
    dp[2] = 3 # 너비 2일 때는 경우의 수 3개임

    for i in range(3, n + 1): # 3부터는 점화식 적용
        dp[i] = dp[i-2] * 2 + dp[i-1] # dp[i-2]*(너비 2인 타일 채우는 경우의수=2) + dp[i-1]*(너비 1인 타일 채우는 경우의수=1)
        dp[i] %= MOD    # 중간 과정에서 MOD연산을 통해 숫자를 줄임

    return dp[n] # n에 대한 방법의 수 리턴 

n = int(input()) # n 입력받기 

print(fill_tile(n)) # n에 대한 방법의 수 출력 

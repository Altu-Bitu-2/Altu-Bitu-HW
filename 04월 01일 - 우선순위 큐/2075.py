...
n = int(input()) # 구해야 하는 수가 몇 번째 큰 수인지 

# 첫 줄은 n이니까 그대로 받아서 
min_heap = list(map(int, input().split()))
hq.heapify(min_heap) # min heap을 최소 힙으로 만들기  

# 이후 n-1 줄에 대해 들어오는 수들을 검사
for _ in range(n-1):
    line = map(int, input().split()) # 다음 줄 
    for x in line: # 줄의 수들을 탐색하면서 
...
print(min_heap[0]) # min heap의 크기가 n이므로 첫 번째 원소 출력 
